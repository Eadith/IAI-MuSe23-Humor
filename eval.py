...
from config import MIMIC_LABELS, MIMIC, PERSONALISATION, device
...
def write_predictions(task, full_metas, full_preds, full_labels, prediction_path, filename):
    ...
    metas_flat = []
    for meta in full_metas:    # 展平 (23 * 512 * d)  --> (11320, d)  最后一个维度没有 512.
    ...
    with torch.no_grad():
        for batch, batch_data in enumerate(data_loader, 1):
            ...
            if use_gpu:
                ...
                feature_lens = [feature_len.cuda() for feature_len in feature_lens]
                labels = labels.cuda()
...
def evaluate(task, model, data_loader, loss_fn, eval_fn, use_gpu=False, predict=False, prediction_path=None,
    ...
    with torch.no_grad():
        for batch, batch_data in enumerate(data_loader, 1):
            ...
            batch_size = features[0].size(0) if task != PERSONALISATION else 1
            ...
            feature_lens = feature_lens[0].detach().cpu().tolist()    # pass
            cutoff = feature_lens[0] if task == PERSONALISATION else batch_size
            if predict:
