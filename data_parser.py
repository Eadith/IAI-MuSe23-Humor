...
def load_humor_subject(feature, subject_id, normalizer) -> Tuple[List[np.ndarray], np.ndarray, np.ndarray]:
    ...
    :return: features, labels, metas.
        ...
        # metas is a ndarray of shape (len(features), 1, 1+len(label columns)) (segment_id, seq_start, seq_end, segment_id)     感觉有问题
        metas is a ndarray of shape (len(features), 1, len(label columns)-1) (subj_id, seq_start, seq_end, segment_id) 
    """
    # parse labels
