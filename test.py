...
def parse_args():
    ...
    parser.add_argument('--cache', action='store_true',    # action='store_true',
    ...
    parser.add_argument('--eval_seed', type=str,  default="105",
...
def main(args):
    ...
    # Evaluate existing model (No training)
    ...
    if args.predict:  # Make predictions for the test partition; this option is set if there are no test labels
        ...
        evaluate(args.task, best_model, data_loader['test'], loss_fn=loss_fn, eval_fn=eval_fn,
                    use_gpu=args.use_gpu, predict=True, prediction_path=args.paths['predict'], filename='predictions_test_ori.csv')
...
if __name__ == '__main__':
    ...
    args.log_file_name = '{}_{}_[{}]_[{}]_[{}_{}_{}_{}]_[{}_{}]'.format('RNN',
        ...
        '{}_{}_[{}]_[{}_{}]'.format('TEMMA', datetime.now(tz=tz.gettz()).strftime("%Y-%m-%d-%H-%M"), "-".join(args.feature), args.lr,args.batch_size)   # 模型参数在这里添加以下，用于区分
