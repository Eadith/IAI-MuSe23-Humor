...
class MuSeDataset(Dataset):
    def __init__(self, data, partition):
        """ feature --> multi-modality features """
        ...
        # for feature in features:    # 都是4或者小于4
        #     feature_lens.append(len(feature))

        """ """
        for feature in features:
            feature_len = []
            for feature_item in feature:    # 4 and less than 4
        ...
        max_feature_len = [np.max(np.array(feature_len)) for feature_len in feature_lens]    # 应该都是4
        self.feature_len_in_one_step = np.max(np.array(max_feature_len))    # 4 


        max_label_len = np.max(np.array(label_lens))
        if max_label_len > 1:   # pass
            assert(max_feature_len[0]==max_label_len)

        # self.feature_lens = torch.tensor(feature_lens)
        self.feature_lens = [torch.tensor(feature_len) for feature_len in feature_lens]    # list of tensor
        ...
        features = [[np.pad(f, pad_width=((0, max_feature_len[i]-f.shape[0]),(0,0))) for f in features[i]] for i in range(len(features))]    # 每个模态都做填充
